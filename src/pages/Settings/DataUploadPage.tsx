...
const DataUploadPage: React.FC = () => {
  ...
  const parseExcelOrCsv = async (file: File) => {
    ...
    // Excel reading for extreme volume (disabling formula parsing for speed)
  ...
  const startUpload = async () => {
    ...
    if (!profile?.company_id) { alert('기업 정보 로딩 실패. 새로고침 후 다시 시도하세요.'); return; }
    ...
    try {
      ...
      if (rows.length === 0) {
        setResult({ total: 0, success: 0, failed: 0, errors: ["파일 데이터가 비어 있습니다."] });
      ...
      for (const dName of uniqDivs) {
          if (!localOrg.divisions[dName]) {
              ...
              created?.forEach(d => { localOrg.divisions[d.name] = d.id; });
          }
      }
      ...
      for (const teamKey of uniqTeams) {
          ...
          if (dId && !localOrg.teams[`${dId}_${tName}`]) {
              ...
              created?.forEach(t => { localOrg.teams[`${t.division_id}_${t.name}`] = t.id; });
          }
      }
      setUploadProgress(25);

      const uniqueStaff = [...new Set(rows.map(r => `${r!.divisionName}||${r!.teamName}||${r!.name}`).filter(k => k.split('||')[2]))];
      const staffChunks = 500;
      for (let i = 0; i < uniqueStaff.length; i += staffChunks) {
          const chunk = uniqueStaff.slice(i, i + staffChunks).filter(k => {
              const [d, t, s] = k.split('||');
              const dId = localOrg.divisions[d];
              const tId = localOrg.teams[`${dId}_${t}`];
              return tId && !localOrg.staff[`${tId}_${s}`];
          });
          if (chunk.length > 0) {
              const { data: created } = await supabase.from('sales_staff').insert(chunk.map((k, idx) => {
                  const [d, t, s] = k.split('||');
                  const tId = localOrg.teams[`${localOrg.divisions[d]}_${t}`];
                  return { team_id: tId, name: s, display_order: Object.keys(localOrg.staff).length + idx + i };
              })).select();
              created?.forEach(s => { localOrg.staff[`${s.team_id}_${s.name}`] = s.id; });
          }
      }
      ...
      for (const cName of uniqCats) {
          if (cName !== '미분류' && !localOrg.categories[cName]) {
              ...
              created?.forEach(c => { localOrg.categories[c.name] = c.id; });
          }
      }
      if (!localOrg.categories['미분류']) {
          const { data: nCat } = await supabase.from('product_categories').insert({ company_id: profile.company_id, name: '미분류', display_order: 999 }).select().single();
          if (nCat) localOrg.categories['미분류'] = nCat.id;
      }
      ...
      rows.forEach(row => {
          ...
          if (!r.date) { errList.push(`${r._rowIndex}행: 날짜 파생 불가(${r.rawDate})`); return; }
          if (!dId || !tId || !sId) {
              errList.push(`${r._rowIndex}행: 조직 불일치 (${r.divisionName} > ${r.teamName} > ${r.name})`);
              return;
          }
      ...
      for (let i = 0; i < tChunks; i++) {
        ...
        if (error) errList.push(`청크(${i + 1}/${tChunks}): ${error.message}`);
    ...
    } catch (e: any) { alert(`중지: ${e.message}`); } finally { setIsUploading(false); }
  };
    ...
    <div className={`${styles.container} fade-in`}>
      <header className={styles.header}>
        <div className={styles.titleArea}>
          ...
          <div>
            <h1 className={styles.title}>대용량 지능형 데이터 업로드 (v2.1)</h1>
            <p className={styles.subtitle}>25만 건 이상의 초대형 마감 데이터도 순차 저장 시스템으로 처리합니다.</p>
          </div>
        </div>
      </header>
      ...
      <div className={styles.uploadCard}>
        {!file ? (
          <div className={`${styles.dropzone} ${isDragging ? styles.isDragging : ''}`} onClick={() => fileInputRef.current?.click()} onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop}>
            ...
            <div className={styles.dropzoneText}><p>{isDragging ? '여기에 놓으세요' : '파일을 선택하거나 드래그하세요'}</p><span>Excel(.xlsx) 25만 건+ 지원</span></div>
        ...
        <div className={styles.instructions}>
          <h3 className={styles.instructionTitle}>⚡ 스마트 데이터 처리 (Safe & Reliable)</h3>
          <ul className={styles.instructionList}>
            <li className={styles.instructionItem}><b>대용량 세이프가드:</b> 25만 건 이상의 데이터는 500건 단위 자동 분할 저장됩니다.</li>
            <li className={styles.instructionItem}><b>스마트 날짜 파싱:</b> 2017-01-01, 2024.03.01, 20240301 등 2017년 이후 모든 형식을 지원합니다.</li>
            <li className={styles.instructionItem}><b>자동 인프라 구성:</b> 엑셀의 사업부/팀/사원이 없으면 자동으로 시스템에 등록됩니다.</li>
            <li className={styles.instructionItem}><b>데이터 병합:</b> 동일 날짜/거래처/품목 실적은 최신 값으로 업데이트됩니다.</li>
          </ul>
          <button className={styles.downloadTemplate} onClick={downloadXlsxTemplate}><Download size={14} /> 2017년형 분석 템플릿 다운로드 (.xlsx)</button>
        </div>
        ...
        <button className={styles.uploadBtn} disabled={!file || isUploading} onClick={startUpload}>
          ...
          {isUploading ? `마감 데이터 순차 매핑 중... ${uploadProgress}%` : '대용량 마감 데이터 업로드 시작'}
        </button>
        ...
        {result && (
          <div style={{ marginTop: 24, width: '100%' }}>
            <div className={styles.statsArea}>
              ...
              <div className={`${styles.statCard} ${styles.success}`}><span className={styles.statLabel}>정상 임포트</span><span className={styles.statValue} style={{color: '#10B981'}}>{result.success}</span></div>
            ...
            {result.errors.length > 0 && <div className={styles.errorArea}><div className={styles.errorTitle}><AlertCircle size={18} /><h4>분역 및 상세 실패 사유 (v2.1 상세)</h4></div><ul className={styles.errorList}>{result.errors.slice(0, 100).map((err, idx) => <li key={idx}>{err}</li>)}</ul></div>}
          </div>
        )}
      </div>
    </div>
